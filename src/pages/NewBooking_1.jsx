...
export default function NewBooking() {
  ...
  const { data: projects = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .from('projects')
        .select(
          'id, name, guideline_value_sqft, sale_rate_per_sqft, unit_of_measure, is_jv, incentive_amount_per_plot'
        )
        .order('name');
  ...
  const { data: landowners = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('landowner_name, share_pct')
        .eq('project_id', projectId)
        .order('sort_order');
      if (error) throw error;
      return data;
  ...
  // ---- Calculations ----
  // JV projects (one or more landowners): total consideration splits into
  //   Landowner Share (GLV) + Company (KSR) Share.
  // Non-JV projects: KSR owns the land outright — no GLV carve-out,
  //   100% of total consideration is company share.
  const calc = useMemo(() => {
    ...
    const totalBeforeDiscount = area * effectiveRate + premium;
    const totalConsideration = totalBeforeDiscount - discount;
